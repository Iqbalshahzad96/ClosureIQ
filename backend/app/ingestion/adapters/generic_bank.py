...
class GenericBankStatementAdapter(BaseAdapter):
    ...
    def parse_file(self, file_bytes, filename, options=None):
        ...
        try:
            ...
            if hasattr(exc, "candidates"):
                msg = f"Missing required columns: Date and Amount. Closest header found was: {exc.candidates[0] if exc.candidates else 'None'}"
    ...
    def map_to_canonical(
        ...
        for row in raw_rows:
            ...
            tx_data = {
                "bank_account_id": bank_account_id or options.get("bank_account_id") or options.get("bank_account"),
                "bank_name": options.get("bank_name"),
                "account_name": options.get("account_name_raw"),
